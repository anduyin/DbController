 ...
 *  */
	...
	$query = "select loan_name from overdue_analysis_track_plus_money_tg group by loan_name";
	$result = mysqli_query($link, $query);
	$arr = $result->fetch_all(MYSQLI_ASSOC);//查询所有借款类型
	$query1 = "select field from overdue_analysis_track_plus_money_tg group by field";
	mysqli_query($link,'set names utf8');
	$result = mysqli_query($link, $query1);
	$arr1 = $result->fetch_all(MYSQLI_ASSOC);//查询所有field字段
	$sql = "select update_time from overdue_analysis_track_plus_money_tg group by update_time";
	mysqli_query($link,'set names utf8');
	$dateResult = mysqli_query($link, $sql);
	$dateInfo = $dateResult->fetch_all(MYSQLI_ASSOC);//查询所有更新时间
	rsort($dateInfo);
	$date = $dateInfo[0]['update_time'];
	$query2 = "select * from overdue_analysis_track_plus_money_tg where loan_name = \"总计\" and field= \"客户端来源\" and update_time =\"$date\"";
...
<body style="background-color:#fff;">
	...
	<div style="text-align: center" class="top">
		...
		<form action="../download_analyze.php" method="post">
            ...
            <span style="font-size:18px;color:#F44B2A;float:left;">金额-回溯版</span>
			<!-- 类别选项 -->
			...
			<input type='hidden' name ="tmp" value='逾期分析-金额-回溯版'>
			<a href="../overdue_analysis_track_plus_num_tg/oatpnt.php" class="href">笔数-回溯版</a>
		</form>
	</div>
	<div id="t">
		<div class="title">
		...
		<table class="table table-hover table-bordered" border="1" style="color: #333;border-collapse:collapse;border-spacing: 0;text-align: center;margin:0;border:1px solid #e5e5e5;">
			<thead>
				<tr>
					...
					<th width="100" class="active">应还金额(万元)</th>
					<th width="100" class="active">当天还款率(%)</th>
					<th width="100" class="active">提前还款率(%)</th>
					<th width="100" class="active">放款金额 (万元)</th>
			...
			<tbody id="tbody">
				...
				<tr>
					...
					<td width="100" class="active"><?php echo number_format((($v['should_money'])/10000),2,'.','') ?></td>
					...
					<td width="100" class="active"><?php echo number_format((($v['repay_money'])/10000),2,'.','')?></td>
					<td width="100" class="active"><?php echo number_format((($v['repay_money_proportion'])*100),2,'.','')?></td>
		...
		$.ajax({
			...
			success:function(re){				
				...
				for(var i in arr){
					...
					var repay_money_proportion  = arr[i]['repay_money_proportion'];
					...
					str +="<td width='100' class='active'>"+should_money+"</td>";
					str +="<td width='100' class='active'>"+onday_ratio+"</td>";
					str +="<td width='100' class='active'>"+prepay_ratio+"</td>";
					str +="<td width='100' class='active'>"+repay_money+"</td>";
					str +="<td width='100' class='active'>"+repay_money_proportion+"</td>";
