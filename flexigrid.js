	...
	$.addFlex = function(t, p) {
		...
		var g = {
			...
			addData: function(data) { //parse data
				...
				if(p.dataType == 'xml')
					p.totalMoney = +$('rows totalMoney', data).text();
				else
					p.totalMoney = data.totalMoney;
					...
					if(typeof(data.rows) == 'undefined') {
						...
						$.each(
							...
							function(i, row) {
								...
								if( row.repay_id){
									$(tr).attr('data-repay_id', row.repay_id);
								}
								//add cell
			...
			buildpager: function() { //rebuild pager based on new properties
				...
				stat = stat.replace(/{totalMoney}/, p.totalMoney);
