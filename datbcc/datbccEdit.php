...
require_once '../Common.php';
    ...
    $date3 = $_POST[2];
    if($date1==$date2){
    	$query = "select * from data_amount_to_be_collected_cg where repay_date= \"$date1\" and status_time = \"$date3\"";
    }else{
    	$query = "select * from data_amount_to_be_collected_cg where repay_date >= \"$date1\" and repay_date <= \"$date2\" and status_time = \"$date3\"";
