 ...
 *  */
	...
	$query = "select status_time from overdue_analysis_track_num_tg group by status_time order by status_time desc";
	$result = mysqli_query($link, $query);
	$arr = $result->fetch_all(MYSQLI_ASSOC);//查询当前最新时间
	$statusTime = $arr[0]['status_time'];
	$query1 = "select field from overdue_analysis_track_num_tg group by field";
	mysqli_query($link,'set names utf8');
	$result = mysqli_query($link, $query1);
	$arr1 = $result->fetch_all(MYSQLI_ASSOC);//查询所有field字段
	$query2 = "select * from overdue_analysis_track_num_tg where status_time= \"$statusTime\" and field= \"贷款名称\"";
...
<body style="background-color:#fff;">
	...
	<div style="text-align: center" class="top">
		...
		<form action="../download.php" method="post">
            ...
            <span style="font-size:18px;color:#F44B2A;float:left;">笔数-回溯版</span>
			<!-- 字段选项 -->
			...
			<input type='hidden' name ="tmp" value='N.xlsx'>
			<a href="../overdue_analysis_track_money/oatm.php" class="href">金额-回溯版</a>
			<a href="oatnPic.php" class="href">柱形图</a>
		</form>
	</div>
	<div id="t">
		<div class="title">
		...
		<table class="table table-hover table-bordered" border="1" style="color: #333;border-collapse:collapse;border-spacing: 0;text-align: center;margin:0;border:1px solid #e5e5e5;">
			<thead>
				<tr>
					...
					<th width="6%" class="active">放款笔数(笔)</th>					
					<th width="6%" class="active">应还笔数(笔)</th>
					...
					<th width="6%" class="active">笔数总占比(%)</th>
				</tr>
			</thead>
			<tbody id="tbody">
				<!-- 第一次表的默认内容  -->
				...
				<tr>
					...
					<td width="6%" class="active"><?php echo $v['count'] ?></td>
					<td width="6%" class="active"><?php echo $v['shouldcount']?></td>
					...
					<td width="6%" class="active"><?php echo number_format((($v['count_proportion'])*100),2,'.','')?></td>
				</tr>
				<?php } ?>
			</tbody>
		</table>
		...
		$.ajax({
			...
			success:function(re){
				...
				for(var i in arr){
					...
					var shouldcount = arr[i]['shouldcount'];
					...
					var count_proportion = arr[i]['count_proportion'];
					var field	=	arr[i]['field'];
					var status_time =arr[i]['status_time'];
					str +="<tr><td width='6%'>"+field_value+"</td><td width='6%' class='active'>"+count+"</td><td width='6%' class='active'>"+shouldcount+"</td><td width='6%' class='active'>"+prepay_ratio+"</td><td width='6%' class='active'>"+onday_ratio+"</td><td width='6%' class='active'>"+T1overdue_ratio+"</td><td width='6%' class='active'>"+T2overdue_ratio+"</td><td width='6%' class='active'>"+T3overdue_ratio+"</td><td width='6%' class='active'>"+T4overdue_ratio+"</td><td width='6%' class='active'>"+T5overdue_ratio+"</td><td width='6%' class='active'>"+T6overdue_ratio+"</td><td width='6%' class='active'>"+T7overdue_ratio+"</td><td width='6%' class='active'>"+T31overdue_ratio+"</td><td width='6%' class='active'>"+T61overdue_ratio+"</td><td width='6%' class='active'>"+T91overdue_ratio+"</td><td width='6%' class='active'>"+count_proportion+"</td></tr>";
				}			
