...
if($time1==$time2){
	$query = "select * from daily_collection_amount_statistics_tg where date = \"$time1\" and update_time = \"$time\"";
}else{
	$query = "select * from daily_collection_amount_statistics_tg where date >= \"$time1\" and date <= \"$time2\" and update_time = \"$time\"";
}
