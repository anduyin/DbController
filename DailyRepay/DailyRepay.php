...
/**
 * 用于显示daily_repayment_amount表的数据
 */
class DailyRepay
{
    private $conn;
    ...
    public function getQuery($start, $end, $isAll=false)
    {
        ...
        if ($isAll) {
            $sql = 'SELECT repay_date, repay_money, prepay_money, on_date_money, not_repay_money, overdue_type_gt0_not_prepay_money FROM  daily_repayment_amount ORDER BY repay_date';
        } else {
            $sql = 'SELECT repay_date, repay_money, prepay_money, on_date_money, not_repay_money, overdue_type_gt0_not_prepay_money FROM  daily_repayment_amount WHERE repay_date BETWEEN :start AND :end ORDER BY repay_date';
        }
        ...
        $sql2 = "SELECT COLUMN_NAME  From information_schema.`COLUMNS` WHERE TABLE_NAME='daily_repayment_amount' AND COLUMN_NAME != 'id' ";
