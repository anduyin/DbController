...
$query1 = "select * from data_accounts_receivable where status_time = \"$status_time1\"";
...
<body>
	...
	<form action="Finance_download.php" method="post">
	<span style="font-size:18px;color:#262626;float:left;margin-left:25px;">财务></span>
    <span style="font-size:18px;color:#F44B2A;float:left;">应收账款</span>
	<!-- 时间选项 -->
	 ...
	 $.ajax({
		 ...
		 success:function(re){
			 ...
			 for(var i in arr){
				 ...
				 var not_repay_money = arr[i]['not_repay_money'];
                 var recover_1 = arr[i]['recover_1'];
                 var recover_30 = arr[i]['recover_30'];
                 var recover_60 = arr[i]['recover_60'];
