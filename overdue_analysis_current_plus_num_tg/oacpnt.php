 ...
 *  */
	...
	$query = "select loan_name from overdue_analysis_current_plus_num_tg group by loan_name";
	$result = mysqli_query($link, $query);
	$arr = $result->fetch_all(MYSQLI_ASSOC);//查询所有借款类型
	$query1 = "select field from overdue_analysis_current_plus_num_tg group by field";
	mysqli_query($link,'set names utf8');
	$result = mysqli_query($link, $query1);
	$arr1 = $result->fetch_all(MYSQLI_ASSOC);//查询所有field字段
	$sql = "select update_time from overdue_analysis_current_plus_num_tg group by update_time";
	mysqli_query($link,'set names utf8');
	$dateResult = mysqli_query($link, $sql);
	$dateInfo = $dateResult->fetch_all(MYSQLI_ASSOC);//查询所有更新时间
	rsort($dateInfo);
	$date = $dateInfo[0]['update_time'];
	$query2 = "select * from overdue_analysis_current_plus_num_tg where loan_name = \"总计\" and field= \"客户端来源\" and update_time =\"$date\"";
...
<body style="background-color:#fff;">
	...
	<div style="text-align: center" class="top">
		...
		<form action="../download_analyze.php" method="post">
            ...
            <span style="font-size:18px;color:#F44B2A;float:left;">笔数-现状版</span>
			<!-- 类别选项 -->
			...
			<input type='hidden' name ="tmp" value='逾期分析-笔数-现状版'>
			<a href="../overdue_analysis_current_plus_money_tg/oacpmt.php" class="href">金额-现状版</a>
		</form>
	</div>
	<div id="t">
		<div class="title">
