...
$query = "select status_time from data_details_should_be_mat group by status_time order by status_time desc";
$result = mysqli_query($link, $query);
$time = $result->fetch_all(MYSQLI_ASSOC);
$timeFirst = $time[0]["status_time"];
require_once 'ddsbmEdit.php';
?>
...
<body>
	...
	for(var i in arr){
		arr1.push(arr[i]['advance_payment']);
		arr2.push(arr[i]['ture_advance_payment']);
		arr3.push(arr[i]['not_advance_payment']);
		arr4.push(arr[i]['difference_in_amount']);
		arr5.push(arr[i]['repay_date']);
	}
	var brr = {};
	brr['arr1'] = arr1;
	brr['arr2'] = arr2;
